using Revise,
    Synapse, Random, Plots, PiecewiseDeterministicMarkovProcesses, ColorSchemes, Sundials
...
if data_protocol[!, :post_poisson_rate][k] > 0.0 #for Poisson, mandatory positive rate, the regular rate in the parameters is used to set the event_time end
    post = Synapse.PoissonProcess(
        data_protocol[!, :post_poisson_rate][k],
        start,
        events_times[end],
    )
    post_ = repeat([0], inner = length(post))
    pre = Synapse.PoissonProcess(
        data_protocol[!, :pre_poisson_rate][k],
        start,
        events_times[end],
    )
...
param_synapse = SynapseParams(
    t_end = start +
            (data_protocol[!, :repeat_times][k] + 1) * pls * 1000 /
            data_protocol[!, :freq][k] +
            .7e3,
    soma_dist = 200.0,
...
is_glu_release, Docked, Reserve, t_stp, glu_release_times, bap_by_epsp_times = stp(
    param_synapse.t_end,
    pre_synapse,
    events_times,
...
gr()
l = @layout [[a{0.5w} b{0.3w}]; [c{0.5w} d{0.3w}]; [e{0.5w} f{0.3w}]; [g{0.5w} h{0.3w}]]
Plots.plot(windowsize = (1.5 * 700 * 0.6, 350 * 1.5), layout = l, grid = false)

#AMPAr
lim = [(start - 15 * 0.02), (start + 15 * 0.28)]
...
plot!(
    ...
    subplot = 1,
    alpha = 1,
    color = get(colorss, 0.0),
    linetype = :steppost,
    layout = l,
    axis = false,
    grid = false,
    xlim = [start - 20, start .+ 250],
    xticks = (collect(500:100:700), collect(0:100:200)),
...
#GABAr
lim = [(start - 100 * 0.015), (start + 100 * 0.3)]
val = XD[49, :] .+ XD[50, :]
plot!(
    tt,
    val,
...
plot!(
    ...
    layout = l,
    xlim = lim,
    xticks = (collect(500:10:530), collect(0:10:30)),
    yticks = (collect(0:10:30)),
)

#NMDAr
lim = [(start - 550 * 0.06), (start + 550)]
val = XD[22, :] .+ XD[23, :]
plot!(
    tt,
    val,
...
plot!(
    ...
    layout = l,
    xlim = [500 - 5, 600],
    xticks = (collect(500:25:600), collect(0:25:100)),
    yticks = (collect(0:2:8)),
)
val = XD[44, :] .+ XD[45, :]
plot!(
    tt,
...
gr()
l = @layout [[a{0.5w} b{0.5w}]; [c{0.5w} d{0.5w}]]
Plots.plot(windowsize = (1.5 * 700 * 0.6, 350 * 1), layout = l, grid = false)
...
for i = 1:5
    ...
    args = (
        ...
        alpha = 0.3,
        w = 2,
        xticks = (collect(500:25:550), collect(0:25:50)),
    )
...
p = param_synapse.p_release
pre_synapse = PreSynapseParams(
    h = (p[4] + p[3] / (1 + exp(p[1] * (data_protocol[!, :exca][k] - p[2])))),
)
...
scatter!(
    glu_release_times / 1000,
    31 .* ones(length(glu_release_times));
    label = "releases",
    subplot = 3,
    xlabel = "time (s)",
    ylabel = "Vesicles",
...
CaMKII =
    out[:KCaM0] .+ out[:KCaM2C] .+ out[:KCaM2N] .+ out[:KCaM4] .+ out[:PCaM0] .+
    out[:PCaM2C] .+ out[:PCaM2N] .+ out[:PCaM4] .+ out[:P] .+ out[:P2]
