...
plot!(w = 0, SynapseParams().LTP_region, color = "red", alpha = 0.1)
plot!(w = 0, SynapseParams().LTD_region, color = "blue", alpha = 0.1);
plot!(grid = false);
...
for k = 2:8
    ...
    plot!(
        tt / 1e3,
        [out[:lt]][1:end][1][1:end, 3] - [out[:lt]][1:end][1][1:end, 2];
        subplot = 2,
        ylabel = "weight change (%)",
        xlabel = "Time (s)",
        label = "$(data_protocol[k,:protocol])",
        ylim = [-80, 90],
        args...,
    ) |> display
end
