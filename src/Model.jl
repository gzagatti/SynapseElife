...
function F_synapse(dxc, xc, xd, p_synapse::SynapseParams, t, events_bap, bap_by_epsp)
	...
	LTD, LTP, act_D, act_P, m, h, n, SK, λ_age, λ_aux = xc
...
function J_synapse(p_synapse::SynapseParams, nu)
	...
	jumps = JumpSet(;
		...
		variable_jumps = [
			################### R-type VGCC ###################
			@j_jump(56, p_synapse, nu, p.xd[25] * alpha_m_r * frwd_VGCC, p.xd[25] * max_m_r * frwd_VGCC, typemax(Float64)), # 56
			@j_jump(57, p_synapse, nu, p.xd[26] * beta_m_r  * bcwd_VGCC, p.xd[26] * max_m_r * frwd_VGCC, typemax(Float64)), # 57
			@j_jump(58, p_synapse, nu, p.xd[25] * alpha_h_r * frwd_VGCC, p.xd[25] * max_h_r * frwd_VGCC, typemax(Float64)), # 58
			@j_jump(59, p_synapse, nu, p.xd[27] * beta_h_r  * bcwd_VGCC, p.xd[27] * max_h_r * bcwd_VGCC, typemax(Float64)), # 59
			@j_jump(60, p_synapse, nu, p.xd[26] * alpha_h_r * frwd_VGCC, p.xd[26] * max_h_r * frwd_VGCC, typemax(Float64)), # 60
			@j_jump(61, p_synapse, nu, p.xd[28] * beta_h_r  * bcwd_VGCC, p.xd[28] * max_h_r * bcwd_VGCC, typemax(Float64)), # 61
			@j_jump(62, p_synapse, nu, p.xd[27] * alpha_m_r * frwd_VGCC, p.xd[27] * max_m_r * frwd_VGCC, typemax(Float64)), # 62
			@j_jump(63, p_synapse, nu, p.xd[28] * beta_m_r  * bcwd_VGCC, p.xd[28] * max_m_r * bcwd_VGCC, typemax(Float64)), # 63


			################### T-type VGCC  ###################
			@j_jump(64, p_synapse, nu, p.xd[29] * alpha_m_t * frwd_VGCC, p.xd[29] * max_m_t * frwd_VGCC, typemax(Float64)), # 64
			@j_jump(65, p_synapse, nu, p.xd[30] * beta_m_t  * bcwd_VGCC, p.xd[30] * max_m_t * bcwd_VGCC, typemax(Float64)), # 65 this one can have a high rate
			@j_jump(66, p_synapse, nu, p.xd[29] * alpha_h_t * frwd_VGCC, p.xd[29] * max_h_t * frwd_VGCC, typemax(Float64)), # 66
			@j_jump(67, p_synapse, nu, p.xd[31] * beta_h_t  * bcwd_VGCC, p.xd[31] * max_h_t * bcwd_VGCC, typemax(Float64)), # 67
			@j_jump(68, p_synapse, nu, p.xd[30] * alpha_h_t * frwd_VGCC, p.xd[30] * max_h_t * frwd_VGCC, typemax(Float64)), # 68
			@j_jump(69, p_synapse, nu, p.xd[32] * beta_h_t  * bcwd_VGCC, p.xd[32] * max_h_t * bcwd_VGCC, typemax(Float64)), # 69
			@j_jump(70, p_synapse, nu, p.xd[31] * alpha_m_t * frwd_VGCC, p.xd[31] * max_m_t * frwd_VGCC, typemax(Float64)), # 70
			@j_jump(71, p_synapse, nu, p.xd[32] * beta_m_t  * bcwd_VGCC, p.xd[32] * max_m_t * bcwd_VGCC, typemax(Float64)), # 71, this one can have a high rate

			################### L-type VGCC  ###################
			@j_jump(72, p_synapse, nu, p.xd[33] * alpha_l  * frwd_VGCC, p.xd[33] * max_alpha_l  * frwd_VGCC, typemax(Float64)), # 72
			@j_jump(73, p_synapse, nu, p.xd[34] * beta_1_l * bcwd_VGCC, p.xd[34] * max_beta_1_l * bcwd_VGCC, typemax(Float64)), # 73
			@j_jump(74, p_synapse, nu, p.xd[33] * alpha_l  * frwd_VGCC, p.xd[33] * max_alpha_l  * frwd_VGCC, typemax(Float64)), # 74
			@j_jump(75, p_synapse, nu, p.xd[35] * beta_2_l * bcwd_VGCC, p.xd[35] * max_beta_2_l * bcwd_VGCC, typemax(Float64)), # 75
...
function SynapseProblem(xc, xd, t1, t2, events_bap, bap_by_epsp, glu, p_synapse, nu, algo::T, agg = nothing; saveat = [], save_everystep = isempty(saveat), kwargs...) where {T <: CHV}
...
function SynapseProblem(xc, xd, t1, t2, events_bap, bap_by_epsp, glu, p_synapse, nu, algo, agg; jumps = nothing, save_positions = (false, true), saveat = [], save_everystep = isempty(saveat), kwargs...)
	p = (xd0 = copy(xd), xd = copy(xd), Glu = p_synapse.glu_amp * glu, p_synapse = p_synapse)
	oprob = ODEProblem((dxc, xc, p, t) -> F_synapse(dxc, xc, p.xd, p.p_synapse, t, events_bap, bap_by_epsp), xc, (t1, t2), p)
	xdsol = SavedValues(typeof(t1), typeof(xd))
	if typeof(saveat) <: Number
		saveat = t1:saveat:t2
	end
        callback = _SavingCallback((u, t, integrator) -> copy(integrator.p.xd), xdsol; saveat, save_everystep)
	dep_graph = buildRxDependencyGraph(nu)
	jprob = JumpProblem(oprob, agg, jumps; dep_graph, save_positions, saveat, save_everystep, callback)
